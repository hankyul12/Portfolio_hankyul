...
저는 kdt에서 자바 스프링 리액트로 완성하는 클라우드 활용 풀스택 개발 과정 수료하였으며, 이 수업에서 8개월간 3차프로젝트까지 진행하며 자바풀스택개발 역량을 강화시켰습니다.
...
- **JSP/Servlet**: JSP 형태 개발 경험
- **React**: React를 활용한 사용자 인터페이스 설계 경험
- **Spring**: Spring Boot 기반 REST API 개발 및 배포 경험, Spring Data JPA를 활용한 웹 애플리케이션 설계 가능
- **Flutter**: 다수 플랫폼에 대한 애플리케이션 사용자 인터페이스 설계 경험
...
Canvas API와 JavaScript를 활용해 웹 브라우저에서 동작하는 1945 슈팅 게임 구현.

타이머, 탄막, 각종 오브젝트 구현(키보드입력, 적 충돌감지), 게임의 전반적인 JS을 개발 
...
JAVAFX를 활용하고 JSON 방식의 클라이언트-서버 통신방식으로 다양한 공간을 예약하고 대여할 수 있는 플랫폼 구현

DB설계, SQL문 작성, 클라이언트와 서버 연결작업, 로그인 메인화면, 마이페이지, 검색, 예약, 결제 화면 클라이언트 개발 

---

### 3. 위치 기반 도우미 매칭  (4인 개발)   [프로젝트 장려상 수상]
...
Flutter를 활용하여 Spring서버와 RestAPI로 통신하는 사용자의 위치를 기반으로 도우미를 매칭해주는 애플리케이션 구현

DB설계, SQL문 작성, 클라이언트와 서버 연결 작업, 마이페이지, 매칭 화면, 수락/거절기능 클라이언트 개발
...
HTML5와 Tomcat서버를 활용하여 AJAX를 사용해본 중고 물품을 사고팔 수 있는 거래 플랫폼 구현

DB설계및 연결, JSP, 이미지 업로드 방법 확립, 물품등록, 검색, 흥정, 마이페이지 클라이언트 개발
